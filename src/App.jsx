...
async function fetchFandomImg(pageTitle) {
  ...
  try {
    ...
    // Step 1: get list of images on the page
    ...
    // Step 2: prefer FH6_ prefixed image; fall back to any image
    const fh6img = images.find(t => t.toLowerCase().startsWith("file:fh6_"))
                || images.find(t => t.startsWith("File:") && !t.toLowerCase().includes("logo") && !t.toLowerCase().includes("icon"));
    if (!fh6img) return null;

    // Step 3: resolve to actual CDN URL at 600px
    const r2 = await fetch(`${api}?action=query&titles=${encodeURIComponent(fh6img)}&prop=imageinfo&iiprop=url&iiurlwidth=600&format=json&origin=*`);
...
const WIKI_IMG = {
  ...
  "2023 #64 Nissan Z":                  "Formula Drift 64 Forsberg Racing Nissan Z",
    // GMC
